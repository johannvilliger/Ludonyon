...
import { ajouterArticleBenevole } from "./actions";

export function AjouterArticleBenevoleForm() {
  ...
  function ajouter() {
    ...
    startTransition(async () => {
      try {
        await ajouterArticleBenevole(nom, Number(prix));
        setNom("");
        setPrix("");
        router.refresh();
      } catch (err) {
  ...
  return (
    <div className="mt-4 rounded-md border border-zinc-200 p-4">
      <p className="text-sm font-medium text-zinc-700">Ajouter un article</p>
